...
- **The absence of a delivery path.** The repository ships no `buildexe.py`, no installer, no Flatpak script and no DMG script; the README says to run it from a clone. That is a missing capability rather than an internal concern: building it adds behaviour, which is the opposite of everything else this file records. It is tracked as its own release unit instead, so it is neither lost nor counted here.
