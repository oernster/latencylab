...
class TaskInstance:
    ...
    parent_task_instance_id: int | None  # event-causality
...
def _sample_duration_ms(rng: np.random.Generator, dist: str, params: dict[str, float]) -> float:
    if dist == "fixed":
...
def _deterministic_sort_key(task_name: str, context_name: str, instance_id: int) -> tuple:
    return (context_name, task_name, instance_id)
...
def simulate_one(
    *,
    ...
    ctx_queues: dict[str, deque[int]] = {name: deque() for name in model.contexts}
    ...
    # Completion heap entries are (end_time, context, task_name, instance_id, slot_index)
    ...
    def occur_event(name: str, time_ms: float, source_task_instance_id: int | None) -> None:
        ...
        for task_name in model.wiring.get(name, ()):  # unwired events are allowed
            enqueue_task(task_name=task_name, enqueue_time_ms=time_ms, parent_task=source_task_instance_id)

    def enqueue_task(task_name: str, enqueue_time_ms: float, parent_task: int | None) -> None:
        ...
        ctx_queues[task_def.context].append(
            (task_name, enqueue_time_ms, parent_task)
        )
    ...
    while completion_heap and not failed:
        ...
        for _, ctx_name, task_name, instance_id, slot in completed:
            ...
            for ev in inst.emitted_events:
                occur_event(ev, time_ms=inst.end_time_ms, source_task_instance_id=instance_id)
    ...
    # Critical path: backtrack from the task instance that defines the makespan.
    critical_tasks: list[str] = []
    critical_ms = makespan
    if instances:
        # Choose the latest end-time instance deterministically.
        last_inst = max(
            instances.values(),
        ...
        while cur is not None:
            ...
            # Determine which predecessor constrained cur.start_time.
            cap_pred = (
            ...
            evt_time = cur.enqueue_time_ms  # equals evt_pred.end_time_ms for causal tasks
